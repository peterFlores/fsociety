...
CREATE PROCEDURE update_user(PUSER_NAME VARCHAR(50),PUSER_NICKNAME VARCHAR(50), PUSER_MAIL VARCHAR(80), PUSER_PASSWORD VARCHAR(255),PUSER_BIRTHDATE DATE,PUSER_GENDER VARCHAR(2),PUSER_ID INT)
...
DELIMITER $$
mysql> CREATE PROCEDURE TOP_5_VISIT()
    -> BEGIN 
    -> SELECT U.USER_ID, U.USER_NAME, U.USER_IMAGE_PATH, COUNT(V.USER_ID) AS SUMA, V.VISIT_DATE FROM USERS U INNER JOIN VISIT V ON U.USER_ID = V.USER_ID WHERE ( V.VISIT_DATE = curdate()) GROUP BY U.USER_ID, U.USER_NAME, U.USER_IMAGE_PATH, V.VISIT_DATE ORDER BY SUMA DESC LIMIT 5; 
    -> END
    -> $$
