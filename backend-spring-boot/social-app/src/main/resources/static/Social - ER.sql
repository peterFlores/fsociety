...
AS
SELECT p.USER_ID AS USER_ID, u.USER_NICKNAME AS USER_NICKNAME, COUNT(p.POST_ID) AS POSTS_COUNT FROM POST p
...
CREATE VIEW V_TOP5_VISITS_WEEKLY
AS
SELECT U.USER_NAME, COUNT(V.USER_ID) AS USER_COUNT
FROM USERS U 
INNER JOIN VISIT V ON U.USER_ID = V.USER_ID
WHERE ( date(V.VISIT_DATE) >= DATE_SUB(SYSDATE(), INTERVAL 7 DAY)) 
GROUP BY U.USER_NAME ORDER BY USER_COUNT DESC LIMIT 5; 

SELECT * FROM V_TOP5_VISITS_WEEKLY;
